...
double **copyMatrix(double **matirix, int n, int m){
...
double **thomas(double **A, int n) {
    auto res = copyMatrix(A, n, 3);
    for(int i=1; i<n; i++){
        res[i][1] -= A[i][0] * A[i-1][2] / res[i-1][1];
        res[i][0] = 0;
    }
    return res;
}

void vectorB(double **A, double **matrixThomas, double *b, int n){
    for(int i=1; i<n; i++){
        b[i] -= A[i][0] * b[i-1] / matrixThomas[i-1][1];
    }
}
